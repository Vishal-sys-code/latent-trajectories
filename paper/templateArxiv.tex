...
The Trajectory Convergence Index $\text{CI}(l)$ (Figure~\ref{fig:convergence_score}) is near zero in early layers ($l \leq 3$ in GPT-2) and rises sharply beginning at the midpoint of the network, reaching peak values of $0.41 \pm 0.03$ (GPT-2, layer 9), $0.58 \pm 0.04$ (TinyLlama, layer 18), and $0.53 \pm 0.03$ (Qwen2.5, layer 24). Under control C1 (random category labels), CI collapses to $0.02 \pm 0.01$ across all layers and all models ($p < 0.001$, $d = 2.31$), confirming that the observed convergence reflects learned semantic structure rather than geometric coincidence. Control C3 (shuffled layers) eliminates the monotonic rise in CI, confirming that the layer ordering, not merely the set of representations is responsible for the observed convergence trajectory.
...
\begin{figure}[htbp]
    ...
    \caption{Trajectory Convergence Index $\text{CI}(l)$ across layers with shaded 95\% bootstrap CIs. The shaded grey band shows the null distribution under C1 (random labels). Non-overlapping CIs in middle-to-late layers confirm statistically significant semantic compression.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{figure6_trajectory_length.png}
    \caption{Total trajectory length $\mathcal{L}(\tau)$ grouped by prompt family, aggregated across all three models. Reasoning prompts (F4) traverse significantly longer paths than lexical variations (F2) ($p < 0.001$, $d > 1.8$). Error bars show 95\% bootstrap CIs.}
...
\begin{itemize}
    ...
    \item \textbf{Phase III - Output Preparation} ($l > \lfloor 3L/4 \rfloor$): A modest secondary drop (0.60--0.70), consistent with the recalibration of representations toward the output vocabulary space observed by~\citet{geva2022transformer}.
\end{itemize}
...
Figure~\ref{fig:app_keyframes} shows five selected keyframes from the layer-by-layer trajectory animation for the \textit{Animals} semantic family under global PCA projection, corresponding to layers 0, 3, 6, 9, and 12 in GPT-2 Small. The full animation (\texttt{figures/trajectory\_animation.gif}) is included in the repository and requires a PDF viewer with animation support (e.g., Adobe Acrobat) or direct browser rendering.

\begin{figure}[t]
\centering

\includegraphics[width=0.40\linewidth]{keyframe_layer0.png}
\includegraphics[width=0.40\linewidth]{keyframe_layer3.png}
\includegraphics[width=0.40\linewidth]{keyframe_layer6.png}

\vspace{0.5em}

\includegraphics[width=0.40\linewidth]{keyframe_layer8.png}
\includegraphics[width=0.40\linewidth]{keyframe_layer11.png}

\caption{
Trajectory keyframes across selected layers of GPT-2 Small.
Representations transition from dispersed configurations in early layers to a compact semantic manifold in later layers.
}
\label{fig:app_keyframes}
\end{figure}
